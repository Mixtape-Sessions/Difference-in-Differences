...
forvalues i = 1/`r(max)' {
		...
		*gen temp=effyear // Create new time_til variables
		*egen treat_date=min(temp)
		gen temp=effyear if group==`i'
		egen treat_date=min(temp)
		drop if effyear<treat_date
		gen event=year-effyear
...
* Defining variables
use ../data/stacking/stacked.dta, replace
